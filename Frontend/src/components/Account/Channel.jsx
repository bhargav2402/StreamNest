...
export default function Channel() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://streamnest-3hxb.onrender.com/api/users/channel/${id}`)
            .then(res => setUser(res.data.data))
            .catch(error => error.response.status >= 500 ? navigate(-1) : console.log(error.response.data))

        axios.get(`https://streamnest-3hxb.onrender.com/api/dashboard/stats/${id}`)
            .then(res => {
    ...
    const toggleSub = () => {
        axios.post(`https://streamnest-3hxb.onrender.com/api/subscription/channel/${user._id}`)
            .then(res => {
