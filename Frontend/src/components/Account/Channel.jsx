...
export default function Channel() {
    ...
    useEffect(() => {
        ...
        axios.get(`https://stream-nest.vercel.app/api/users/channel/${id}`)
            .then(res => setUser(res.data.data))
            .catch(error => error.response.status >= 500 ? navigate(-1) : console.log(error.response.data))

        axios.get(`https://stream-nest.vercel.app/api/dashboard/stats/${id}`)
            .then(res => {
    ...
    const toggleSub = () => {
        axios.post(`https://stream-nest.vercel.app/api/subscription/channel/${user._id}`)
            .then(res => {
